...
# -*- coding: utf-8 -*-
"""
Created on Tue Oct  6 09:24:31 2020

@author: rajeshkanna and thrinad
"""
import json
import os, shutil, glob, os.path, numpy as np, time, tensorflow as tf, tensorflow_hub as hub,json2html
...
def jsontotable(d):
    ...
    for k, v in d.items():
        ...
        if i == 1:
            ...
            txt = txt + """<td><p><img src="/static/img/{0}" /><br/>{1}</p></td><td>{2}%</td></tr>""".format(k, k, str(int(v)))
            i = i + 1
        else:
            txt = txt + """<tr style="text-align: center;vertical-align: middle;"><td></td><td><p><img src="/static/img/{0}" /><br/>{1}</p></td><td>{2}%</td></tr>""".format(k, k, str(int(v)))
...
def jpg_2_png():
        import os
        for dirs,subdirs,files, in os.walk("/home/luminad/Documents/Search_Similar_Images/static/img/"):
            for file in files:
                if file.endswith(".jpg") or file.endswith(".JPG") or file.endswith(".jpeg") or file.endswith(".JEPG") or file.endswith(".PNG"):
                    # print(file)
                    os.rename(os.path.join(dirs,file), os.path.join(dirs,file.split(".")[0]+".png"))
...
def uploaded_file(filename):
    ...
    d = dict(zip(image_names_ext,[i*100 for i in  percentage]))
    
    # del d[next(iter(d))]
    d
    d=json.dumps(d  , sort_keys=False)
    # d=json.dumps(dict(zip(imagename,[i*100 for i in  percentage]))  , sort_keys=False)
                            

    # d=json.dumps(d,indent=4, sort_keys=True)
    # output = json2html.convert(json = d, table_attributes="id=\"info-table\" class=\"table table-striped table-hover\"")
    # print(d)
